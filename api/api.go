...
func NewApi(r *gin.Engine, cfg *config.Config, store storage.StorageI, logger logger.LoggerI) {
	...
	// r.Use(customCORSMiddlewere())

	v1 := r.Group("/v1")

	v1.Use(handler.AuthMiddlewere())
	...
	// order api
	r.GET("/order/:id", handler.AuthMiddlewere(), handler.GetByIdOrder)
	r.POST("/order",handler.AuthMiddlewere(), handler.CreateOrder)
	r.GET("/order",handler.AuthMiddlewere(), handler.GetListOrder)
	r.GET("/order/total_sum",handler.AuthMiddlewere(), handler.OrderTotalSum)
	r.PUT("/order/:id",handler.AuthMiddlewere(), handler.UpdateOrder)
	r.PATCH("/order/:id",handler.AuthMiddlewere(), handler.UpdatePatchOrder)
	r.DELETE("/order/:id",handler.AuthMiddlewere(), handler.DeleteOrder)
	r.POST("/order_item/",handler.AuthMiddlewere(), handler.CreateOrderItem)
	r.DELETE("/order_item/:id",handler.AuthMiddlewere(), handler.DeleteOrderItem)
